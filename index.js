...
;// CONCATENATED MODULE: ./src/lib/next-ts.ts
...
const lastCurPath = () => location.href.match(/[^\/]+$/)?.[0];
const findChunkIndex = (arr, chunk) => {
  const headIdx = arr.findIndex(item => item === chunk[0]);
  const compareChunk = arr.slice(headIdx, headIdx + chunk.length);
  if (!isEqual(chunk, compareChunk)) return -1;
  return headIdx;
};
const filterIdx = (arr, callback) => arr.map((item, i) => callback(item, i) ? i : void 0).filter(item => lodash_es_isInteger(item));
const everyType = (list, condDef) => {
  return list.every(condDef);
};
const findSeparately = (list, callback) => {
  let i = 0;
  for (const item of list) {
    const [cond, res] = callback(item, i);
    if (cond) return res;
    i++;
  }
};
const updateMap = (list, callback) => {
  let i = 0;
...
const unionEqual = (a, b) => union(a, b).length === a.length + b.length;
const unionEqualWith = (a, b, comparetor) => unionWith(a, b, comparetor).length === a.length + b.length;
const parseScript = script => goErr(() => new Function(`"use strict";return (${script})`)());
const isPlainObject = obj => isPlainObjectLodash(obj);
class WebStorage extends (/* unused pure expression or super */ null && (Init)) {
  type = 'local';
  onChange;
  get keys() {
  ...
  get data() {
    return mapValues(this.storage, v => {
      const res = goErr(() => JSON.parse(v));
  ...
  set data(state) {
    // 異常なプロパティを削除
    difference(this.keys, Object.keys(state)).forEach(removeKey => this.storage.removeItem(removeKey));
    entries(state).forEach(([k, v]) => {
      this.storage.setItem(k, typeof v === 'string' ? v : JSON.stringify(v));
    });
    this.onChange?.(this.data);
  }
  merge(input) {
    this.data = mergePlainObj(this.data, input);
  }
  delete() {
    this.keys.forEach(k => this.storage.removeItem(k));
  }
}
// TODO 汎用的な再帰処理回避のメソッド
...
;// CONCATENATED MODULE: ./node_modules/use-onclickoutside/dist/use-onclickoutside.browser.esm.js
...
function use_onclickoutside_browser_esm_useOnClickOutside(ref, handler, _temp) {
...
;// CONCATENATED MODULE: ./node_modules/my-useful-hooks/dist/useClickOutside.js
 // TODO パッケージ先Reactに依存しない代替
/* harmony default export */ const useClickOutside = ((r, handler) => {
...
;// CONCATENATED MODULE: ./node_modules/my-useful-hooks/dist/useNowDayjs.js

// stateの中身を、1秒おきに最新のdayjsに更新
/* harmony default export */ const useNowDayjs = ((r) => {
    const [nowDayjs, setNowDayjs] = r.useState(dayjs_min());
    const interval = r.useRef();
    r.useEffect(() => {
        interval.current = setInterval(() => setNowDayjs(dayjs_min()), 1000);
    }, []);
    return [nowDayjs, interval];
});

;// CONCATENATED MODULE: ./node_modules/my-useful-hooks/dist/useRename.js
 // TODO パッケージ先Reactに依存しない代替

/* harmony default export */ const useRename = ((r, { initial, input, onRename }) => {
...
;// CONCATENATED MODULE: ./node_modules/my-useful-hooks/dist/index.js
...
;// CONCATENATED MODULE: ./node_modules/next-ts-utility/dist/conversion.js
/* harmony default export */ const conversion = ((sec, by) => {
    const res = [sec];
...
;// CONCATENATED MODULE: ./node_modules/next-ts-utility/dist/zeroPad.js
/* harmony default export */ const zeroPad = ((int, padNum) => ('0'.repeat(padNum) + int).slice(-padNum));

;// CONCATENATED MODULE: ./src/bus/TimeRow.tsx


const forShowTime = '秒,分,時間,日,年'.split(',');
const parseTime = times => times.map(time => zeroPad(time, 2)).reverse().join(':');
// 通常は 0 にする。1なら臨時便デバッグ
const temporary = 0;
const TimeRow = ({
  className,
  row,
  ...
  return (0,jsx_runtime.jsxs)("tr", {
    className: `${className ?? ''} ${idx == 0 ? 'h-12 mt-4 align-bottom text-3xl' : 'text-xl'} ${!!~row.tag.indexOf('臨') || temporary ? 'text-red-500	' : ''}`,
    children: [(0,jsx_runtime.jsx)("td", {
      children: parseTime(row.time)
    }), (0,jsx_runtime.jsx)("td", {
      children: row.cmp.map((t, i) => t + forShowTime[i]).reverse().join('')
    }), (0,jsx_runtime.jsx)("td", {
      children: row.tag
    })]
  });
};
/* harmony default export */ const bus_TimeRow = (TimeRow);
;// CONCATENATED MODULE: ./src/bus/App.tsx
...
const forParseTime = [60, 60, 24, 365];
// 正常動作中は、ココを0にする
const maintenanceNotice =  false ? 0 : '';
const App = ({
  timeTables
}) => {
  const [nowDayjs] = useNowDayjs(react);
  const [menuId, setMenu] = (0,react.useState)(1);
  const selectTimeTable = (0,react.useMemo)(() => timeTables?.[menuId], [menuId]);
  const filteredTable = (0,react.useMemo)(() => {
    const [nowMin, nowHour] = [nowDayjs.minute(), nowDayjs.hour()];
    const nowUnix = nowDayjs.unix();
    return selectTimeTable?.table.filter(({
      min,
      hour
    }) => {
      if (hour == nowHour) return nowMin < min;
      return nowHour < hour;
    }).map(({
      min,
      hour,
      ...
      const fixedUnix = nowDayjs.hour(hour).minute(min).second(0).unix();
      return {
        time: [min, hour],
        cmp: conversion(fixedUnix - nowUnix, forParseTime),
        tag
      };
    });
  }, [selectTimeTable, nowDayjs]);
  return !maintenanceNotice.length ? (0,jsx_runtime.jsxs)("div", {
    className: 'flex flex-col w-full h-full text-center',
    children: [(0,jsx_runtime.jsxs)("div", {
      className: 'text-sm',
      children: ["\u3053\u306E\u30DA\u30FC\u30B8\u306E\u8868\u8A18\u304C\u539F\u56E0\u3067\u8D77\u3053\u3063\u305F\u640D\u5BB3\u306B\u3064\u3044\u3066\u306F\u3001\u4E00\u5207\u306E\u8CAC\u4EFB\u3092\u8CA0\u3044\u304B\u306D\u307E\u3059\u3002\u3088\u308A\u6B63\u78BA\u306A\u60C5\u5831\u306F", (0,jsx_runtime.jsx)("a", {
        href: 'https://www.osaka-sandai.ac.jp/life/access_map/suminodo_bus.html',
        target: '_blank',
        children: "[\u3053\u3061\u3089]"
      }), "\u3092\u3054\u78BA\u8A8D\u4E0B\u3055\u3044\u3002"]
      ...
      children: (0,jsx_runtime.jsxs)("table", {
        className: 'w-full',
        children: [(0,jsx_runtime.jsxs)(StickyRow, {
        ...
        }), filteredTable?.map((row, i) => nowDayjs.month() == 2 && !!~row.tag.indexOf('臨') ? null : (0,jsx_runtime.jsx)(bus_TimeRow, {
          idx: i,
          row: row
        }))]
      })
    }), (0,jsx_runtime.jsx)(MenuBar, {
      className: 'grid grid-cols-2',
      children: timeTables?.map(({
        cap
      }, i) => (0,jsx_runtime.jsx)(MenuBtn, {
        onClick: () => setMenu(i),
        className: `text-xl ${menuId == i ? `font-bold ${!!~cap.indexOf('大学発') ? 'bg-amber-300' : 'bg-teal-300'}` : ''}`,
        children: cap
      }, i))
    })]
  }) : (0,jsx_runtime.jsx)("div", {
    className: 'w-full h-full flex items-center\tjustify-center',
    children: maintenanceNotice
  });
};
const MenuBtn = styled_components_browser_esm.button.withConfig({
  displayName: "App__MenuBtn",
  componentId: "sc-1xo23ck-0"
})(["height:3rem;"]);
const StickyRow = styled_components_browser_esm.tr.withConfig({
  displayName: "App__StickyRow",
  componentId: "sc-1xo23ck-1"
})(["position:sticky;top:0;z-index:1;background-color:#fff;"]);
const MenuBar = styled_components_browser_esm.div.withConfig({
  displayName: "App__MenuBar",
  componentId: "sc-1xo23ck-2"
})(["border-top:solid 1px black;"]);
/* harmony default export */ const bus_App = (App);
;// CONCATENATED MODULE: ./src/bus/timeTable.ts
const timeTablesTxt = `
大学発
7,35,45,55
8,5,10,22,32,42,52
...
14,30,臨35,40,50
15,0,20,40,55
16,中10,中15,中20,中臨25,中30,中45,中55
17,中10,中25,中40,中50
18,中0,中10,中20,中臨35,中50
19,中5,中20,中40
20,中5,中25,中45
21,中5,中20,中35,中50
22,中15
住道発
7,15,25,35,45,50
8,中5,中15,中25,中臨30,中35,中40,中50
9,中0,中10,中15,中25,中40,中55
10,中臨0,中5,中10,中15,中35
11,5,20,30
12,0,10,臨15,20,55
13,臨10,30,45
14,0,10,55
...
17,10,25,40
18,0`;
;// CONCATENATED MODULE: ./src/bus/index.tsx
...
// index位置で配列を分割
const divIdx = (arr, idxes) => {
  const res = [];
  if (lodash_es_last(idxes) !== arr.length) idxes.push(arr.length);
  idxes.reduce((acc, cur) => {
    if (acc === cur) return cur;
    res.push(arr.slice(acc, cur));
    return cur;
  }, 0);
  return res;
};
const txtToTable = txt => txt.split('\n').map(rowTxt => rowTxt.replaceAll(' ', '').split(','));
const parseTbl = catedTable => catedTable.map(row => row.map(txt => {
  const mtc = txt.match(/([中臨]*)(\d+)/);
  if (!mtc) return {
    txt
  };
  const num = parseInt(mtc[2]);
  return {
    txt,
    num,
    tag: mtc[1]
  ...
  txt
}) => txt.length)).filter(row => row.length);
const divTbl = parsedTable => {
  const capIdxes = filterIdx(parsedTable, row => !lodash_es_isInteger(row[0]?.num));
  return divIdx(parsedTable, capIdxes).map(tableWithCap => {
    const [capRow, tableRow] = divIdx(tableWithCap, [1]);
    return {
      cap: capRow[0][0].txt,
      table: tableRow
    };
...
const toInlineTbl = table => {
  return table.flatMap(item => {
    const hour = item[0].num;
    if (typeof hour !== 'number') return [];
    const inlineTimeItems = item.slice(1).filter(p => lodash_es_isInteger(p.num) && lodash_es_isString(p.tag)).map(({
      num,
      tag
    }) => ({
      tag,
    ...
    return inlineTimeItems;
  });
};
const timeTbls = divTbl(parseTbl(txtToTable(timeTablesTxt))).map(({
  cap,
  table
}) => ({
  cap,
  table: toInlineTbl(table)
}));
console.log(timeTbls);
const app = Object.assign(document.createElement('div'), {
  id: 'app'
});
document.body.appendChild(app);
const GlobalStyle = We(["html,body,#app{height:100%;}"]);
(0,react_dom.render)((0,jsx_runtime.jsxs)(jsx_runtime.Fragment, {
  children: [(0,jsx_runtime.jsx)(GlobalStyle, {}), (0,jsx_runtime.jsx)(bus_App, {
    timeTables: timeTbls
  }), ' ']
}), app);
})();
